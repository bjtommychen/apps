...
#SINA    
def get_us_rt_price_SinaWeb(code):
    global wd
    code = code.upper()
    url = 'http://stock.finance.sina.com.cn/usstock/quotes/%s.html' % code
    #print url, '---------------------------------------------'
    if wd == None:
        wd = webdriver.Firefox()
        wd.set_window_size(100,100)
    ...
    if len(wd.page_source) < 500:
        print wd.page_source.encode('utf8')
        return ('', 0, 0, 0, 0, 0) 
    a = wd.find_elements_by_class_name('hq_summary')
    if a == []:
        print 'error a', a
        return ('', 0, 0, 0, 0, 0)
    b = a[0].text.split()
    ...
    #print b
    curr = float(b[0])
    d = wd.find_elements_by_class_name('hq_details')[0].text.split()
    if len(d) != 26:
...
def get_us_rt_price_sohu_work(code):
    global wd
    code = code.upper()
    url = 'http://quotes.money.163.com/usstock/%s.html#2u01' % code
    print url, '---------------------------------------------'
...
if  __name__ == '__main__':   
    ...
    while True:
        ...
        print '---- yahoo url ----'
        for one in us_list:
            print get_us_rt_price_yahoo(one)
            time.sleep(1)
