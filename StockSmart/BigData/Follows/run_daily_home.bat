...
python ..\UpdateCSV_withQMdata1min.py -opath output_qda1m -ipath input_qda1m
...
REM delay 10m for Kuaidisk sync done.
REM sleep 5m
cp f:/KuaiDisk/StockSmart/follows/data/*.csv ./data -n
cp f:/KuaiDisk/StockSmart/follows/hold*.csv . -f
cp f:/KuaiDisk/StockSmart/follows/watch*.csv . -f
REM rm -f save_png/*
