...
#include "MP3Decoder_tommy.h"
#define LOG_NDEBUG 0
#define LOG_TAG "MP3DecoderTommy"
...
static bool get_mp3_frame_size(uint32_t header, size_t *frame_size, int *out_sampling_rate = NULL, int *out_channels =
		NULL, int *out_bitrate = NULL)
{
	*frame_size = 0;
...
static bool resync(uint8_t *data, uint32_t size, uint32_t match_header, off_t *out_pos)
{
...
extern "C" void mylog(char *fmt)
{
	//LOGI("mylog");
	__android_log_print(ANDROID_LOG_INFO, "mp3codec", "%s", fmt);
}

static enum MP3D_MSG decode_header(void *data, struct MP3DEC_HEADER const *header)
{
	...
	LOGV(
			" decode header, frame No.%d: mpeg%d, layer%d, %dbps, %dhz.", absolute_framecount, mpegversion, layer, bitrate, sample_rate);
	absolute_framecount++;

	return MP3D_MSG_CONTINUE;
}

status_t MP3DecoderTommy::decode_input_read()
{
	status_t err;
	int offset;
	MediaBuffer *mbtmp;

//	LOGD("enter decode_input_read, unused %d words. new size %d", lstream.unusedsize, lstream.streamsize);

	if (mInputBuffer != NULL)
	{
		offset = mInputBuffer->range_offset() + (mInputBuffer->range_length() - lstream.unusedsize * sizeof(uint16_t));

		memmove(mInputBuffer->data(), mInputBuffer->data() + offset, lstream.unusedsize * sizeof(uint16_t));

		mInputBuffer->set_range(0, lstream.unusedsize * sizeof(uint16_t));
	...
	while (mInputBuffer->range_length() % 2 != 0 || mInputBuffer->range_length() < (MPEG_BUFSZ * sizeof(int16_t)))
	{
		...
		mInputBuffer->set_range(mInputBuffer->range_offset(), mInputBuffer->range_length() + mbtmp->range_length());
		mbtmp->release();
	}

	lstream.pb_stream = (uint16_t*) ((uint8_t *) mInputBuffer->data() + mInputBuffer->range_offset());
	//player->input_length = stream->unusedsize + len;
	lstream.streamsize = (mInputBuffer->range_length()) / sizeof(uint16_t);
//	LOGD(" decode_input_read, unused %d words. new size %d", lstream.unusedsize, lstream.streamsize);

#if 0
	{
		int i;
		uint8_t *p;
		p = (uint8_t *)lstream.pb_stream;
		for(i=0;i<16;i++)
		{
			LOGD(" no.%d, 0x%02x", i, *p++);
		}
	}
#endif
	return OK;
...
MP3DecoderTommy::MP3DecoderTommy(const sp<MediaSource> &source) :
		mSource(source), mNumChannels(0), mStarted(false), mBufferGroup(NULL), mConfig(NULL), //new tPVMP3DecoderExternal),
		mDecoderBuf(NULL), mAnchorTimeUs(0), mNumFramesOutput(0), mInputBuffer(NULL), mPartialBuffer(NULL), mFixedHeader(
				0)
{
	init();
	LOGV(" MP3DecoderTommy()");
}

void MP3DecoderTommy::init()
{
	...
	mMeta->setCString(kKeyDecoderComponent, "MP3DecoderTommy");
}

MP3DecoderTommy::~MP3DecoderTommy()
{
	if (mStarted)
	{
		stop();
...
status_t MP3DecoderTommy::start(MetaData *params)
{
	CHECK(!mStarted);
	...
	initcfg.pCoreMemPool = (unsigned short*) malloc(initcfg.CoreMemPoolSize * sizeof(short));
	if (initcfg.pCoreMemPool == 0)
	{
		LOGE("initcfg.pCoreMemPool malloc return 0!\n");
		//goto errout1;
	}

	mp3decoder = mp3decoder_init(NULL, decode_header, &initcfg);
	if (mp3decoder)
	{
		LOGV(" mp3decoder_init done, mp3decoder handle created !");
		LOGV(" %s ", get_libmp3dec_version());
	}
	memset(&lstream, 0, sizeof(struct MP3DEC_STREAM));
	memset(&lpcm, 0, sizeof(struct MP3DEC_PCM));

	absolute_framecount = 0;
	error_time = 0;
	sync_done = 0;
	...
	LOGV(" MP3DecoderTommy::start()");

	return OK;
}

status_t MP3DecoderTommy::stop()
{
	CHECK(mStarted);
...
sp<MetaData> MP3DecoderTommy::getFormat()
{
	LOGV(" MP3DecoderTommy::getFormat()");
	return mMeta;
}

status_t MP3DecoderTommy::updatePartialFrame()
{
	status_t err = OK;
#if 0
	...
	if (partialBufLen < 4)
	{ // check if partial frame has the 4 bytes header
		if (inputBufLen < (4 - partialBufLen))
		{
			// input buffer does not have the frame header bytes
			// bail out TODO
			LOGE("MP3DecoderTommy::updatePartialFrame buffer to small header not found"
	...
	if (!curFrame)
	{
		LOGE("MP3DecoderTommy::read - partial frame does not have a vaild header 0x%x",
				header);
		return UNKNOWN_ERROR;
	}
	...
	if ((nextFrameOffset + 4) <= inputBufLen)
	{
		...
		if ((header & 0xffe00000) != 0xffe00000)
		{
			...
			LOGE("MP3DecoderTommy::read - next frame does not have a vaild header 0x%x",
					header);
			return UNKNOWN_ERROR;
		}
	}
	else
	{
		...
		LOGE("MP3DecoderTommy::read - assuming next frame is good");
	}
	...
	if (frameSize > (partialBufLen + inputBufLen))
	{
		...
		LOGE("MP3DecoderTommy::updatePartialFrame - input buffer does not have the complete frame."
	...
	if ((mPartialBuffer->size() - partialBufLen) < (frameSize - partialBufLen))
	{
		...
		//TODO
		LOGE("MP3DecoderTommy::updatePartialFrame - mPartialBuffer is small, size %d, required &d",
	...
	LOGE("MP3DecoderTommy::updatePartialFrame - copied the partial frame %d, input buffer length %d",
			bytesRemaining, mInputBuffer->range_length());
#endif 
	return err;
}

status_t MP3DecoderTommy::read(MediaBuffer **out, const ReadOptions *options)
{
#if 0
	status_t err;
	...
	*out = NULL;
	...
	if (mInputBuffer == NULL)
	{
		...
		// check for partial frame    status_t err;
	...
	if ((decoderErr = 0/*pvmp3_framedecoder(mConfig, mDecoderBuf)*/)
			!= NO_DECODING_ERROR)
	{
	...
	*out = buffer;
	...
	{
//		LOGI(" LOOP ");
		err = decode_input_read();
		if (err != OK)
		{	//fixme, do handle ERROR_END_OF_STREAM.
			LOGE("decode_input_read error!");
			buffer->release();
			*out = NULL;
			return err;
		}

		lpcm.pOutBuffer = (uint16_t *) buffer->data();
		//lpcm.OutBufferSize =  buffer->size() / sizeof(int16_t);
		if (buffer->size() > (1152 * 2 * 2))
		{
			lpcm.OutBufferSize = (1152 * 2); //1152*2 for frame, required by mp3dec codec.
		}

		result = mp3decoder_run(mp3decoder, &lstream, &lpcm);
		if (result != MP3D_STATUS_SUCCESS)
		{
			error_time++;
			LOGE("mp3decoder_run result is %d, total is %d !", result, error_time);
			if (error_time > 100)
			{ // avoid dead loop.
				buffer->release();
				buffer = NULL;
		...
		{
//			LOGV("mp3decoder_run result is %d, total is %d !", result, error_time);
//			LOGV("mp3decoder_run got %d samples. ", lpcm.length);
			buffer->set_range(0, lpcm.length * lpcm.channels * sizeof(int16_t));

			if (sync_done == 0)
			{
				sync_done = 1;
				error_time = 0;
				LOGV(" !!! Lost Sync %d times before Sync OK !!! \n", error_time);
			}
		}

	} while (result != MP3D_STATUS_SUCCESS);

	buffer->meta_data()->setInt64(kKeyTime, mAnchorTimeUs + (mNumFramesOutput * 1000000) / sample_rate);
	mNumFramesOutput += lpcm.length;
